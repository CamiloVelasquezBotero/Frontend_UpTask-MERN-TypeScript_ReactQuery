...
export default function NavMenu({name}:NavMenuProps) {
  ...
  const logout = () => {
    queryClient.invalidateQueries({queryKey: ['user']}) // Eliminamos el cacheo de useQuery cuando lo obtuvimos
    localStorage.removeItem('AUTH_TOKEN')
    //queryClient.clear() // Borra todo el cache de React-Query
  }

  return (
    <Popover className="relative lg:mr-20">
